  ...
  if (carrito != null) {
    for (let a of carrito) {
      ...
      contenedor.innerHTML = `
        ...
        <td>${a.nombre}</td>
        <td><a href=""><img width="25" height="25" class="deleteProducto" id=${a.id} src="../imagen/delete.png" alt=""></a></td>
        <td>\$${a.precio}</td>
        `;
...
formaPago.addEventListener("change", (event) => {
  ...
  if (event.target.value == "4") {
    ...
    document.querySelector(".totalFormaPago").innerText =
      "Su total a pagar sera de $" + importeFinal;
  }
});
...
cuotasPago.addEventListener("change", (event) => {
  ...
  importeFinal = importeTotalConRecargo(
    event.target.selectedOptions[0].getAttribute("data-recargo")
  );
  document.querySelector(".totalFormaPago").innerText =
    "Su total a pagar sera de $" + importeFinal;
});
...
irAPago.addEventListener("click", () => {
  ...
  if (
    formaPagoMenu.options[formaPagoMenu.selectedIndex].value == "-1" ||
    (formaPagoMenu.options[formaPagoMenu.selectedIndex].value == "4" &&
      formaPagoCuotas.options[formaPagoCuotas.selectedIndex].value == "-1")
  ) {
...
function obtenerInputs() {
  ...
  document.querySelector(".modal1").innerText =
    "Gracias por su Compra " + nombre + " " + apellido + "!";
  document.querySelector(".modal2").innerText =
    "Su pago se realizo con exito!!!";
  document.querySelector(".modal3").innerText =
    "En breve recibira un mail en la casilla de correo " +
    email +
    " con el detalle del pedido y factura de compra. " + " Recuerde su numero de orden por cualquier consultar ";

  document.querySelector(".btnFinalizar").style.display = "block";

   const numeroOrden = Math.floor(Math.random() * 1000000) + 1;
 document.getElementById("numeroOrden").innerText = numeroOrden;    
